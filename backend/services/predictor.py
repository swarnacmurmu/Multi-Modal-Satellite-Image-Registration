...
class Predictor:
    ...
    def _checkerboard(self, fixed, moving,  block_size=32):
        fixed_u8 = self._to_uint8(fixed)
        moving_u8 = self._to_uint8(moving)

        fixed_color = cv2.cvtColor(fixed_u8, cv2.COLOR_GRAY2BGR)
        moving_color = cv2.cvtColor(moving_u8, cv2.COLOR_GRAY2BGR)

        h, w = fixed_u8.shape
        output = np.zeros_like(fixed_color)

        for y in range(0, h, block_size):
            for x in range(0, w, block_size):
                if ((x // block_size) + (y //   block_size)) % 2 == 0:
                    output[y:y+block_size, x:x+block_size] = fixed_color[y:y+block_size, x:x+block_size]
                else:
                    output[y:y+block_size, x:x+block_size] = moving_color[y:y+block_size, x:x+block_size]
